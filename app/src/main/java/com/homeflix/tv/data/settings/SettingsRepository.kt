...
import com.homeflix.tv.data.stream.DynamicRangeFilter
import com.homeflix.tv.data.stream.MinQualityFilter
import com.homeflix.tv.data.stream.StreamFilters
import com.homeflix.tv.data.stream.VideoCodecFilter
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.first
import kotlinx.coroutines.flow.map

private val Context.dataStore: DataStore<Preferences> by preferencesDataStore(name = "homeflix_settings")
...
class SettingsRepository(private val context: Context) {
    ...
    val tmdbKey: Flow<String> = context.dataStore.data.map {
        it[Keys.TMDB_KEY]?.takeIf { k -> k.isNotBlank() }
            ?: com.homeflix.tv.BuildConfig.TMDB_DEFAULT_TOKEN
    }
    ...
    val mdblistKey: Flow<String> = context.dataStore.data.map {
        it[Keys.MDBLIST_KEY]?.takeIf { k -> k.isNotBlank() }
            ?: com.homeflix.tv.BuildConfig.MDBLIST_DEFAULT_KEY
    }
    ...
    val geminiKey: Flow<String> = context.dataStore.data.map {
        it[Keys.GEMINI_KEY]?.takeIf { k -> k.isNotBlank() }
            ?: com.homeflix.tv.BuildConfig.GEMINI_DEFAULT_KEY
    }
     ...
     * as enum names and resolved to a [com.homeflix.tv.data.stream.StreamFilters]
    ...
    val isConfigured: Flow<Boolean> =
        context.dataStore.data.map {
            (it[Keys.TMDB_KEY].orEmpty()).isNotBlank() ||
                com.homeflix.tv.BuildConfig.TMDB_DEFAULT_TOKEN.isNotBlank()
        }
