...
import com.homeflix.tv.presentation.components.FocusExpandRow
import com.homeflix.tv.presentation.components.Top10Row
import com.homeflix.tv.presentation.components.NAV_BAR_HEIGHT
import com.homeflix.tv.presentation.components.toMediaItem
import com.homeflix.tv.presentation.components.ShimmerLoadingSkeleton
...
private fun BrowseContent(
    ...
    LazyColumn(
        ...
        itemsIndexed(rows, key = { _, r -> "${tab.label}_${r.title}" }) { _, row ->
            ...
            val select: (com.homeflix.tv.presentation.components.MediaItem) -> Unit = { media ->
                row.items.firstOrNull { "${it.type}_${it.id}" == media.id }?.let(onSelect)
            }
            if (row.ranked) {
